...
static float IC_Angle(const Mat& image, Point2f pt,  const vector<int> & u_max)
{
    ...
    // 以像素点为中心，HALF_PATCH_SIZE半径的圆形区域
    for (int u = -HALF_PATCH_SIZE; u <= HALF_PATCH_SIZE; ++u)
        // v = 0 水平轴，因为下面的 v 是从1开始的， v = 0单独计算
        m_10 += u * center[u];

    // Go line by line in the circular patch
    // step1()和step(0)对于二维矩阵是一行元素的总通道数
    // step1(1)对应是单一像素点的通道数
    int step = (int)image.step1();
    for (int v = 1; v <= HALF_PATCH_SIZE; ++v)
    {
        // Proceed over the two lines
        int v_sum = 0;
        // u_max 所选patch每一纵坐标对应的横坐标
        int d = u_max[v];
        for (int u = -d; u <= d; ++u)
        {   // 同一高度左右对应位置像素值
    ...
    // fastAtan2得到质心点和x轴的角度
    return fastAtan2((float)m_01, (float)m_10);
}

// CV_PI 弧度制中角度1度对应的值,(CV_PI/180) *90 即90度角对应的弧度值
const float factorPI = (float)(CV_PI/180.f);
// 计算描述子
static void computeOrbDescriptor(const KeyPoint& kpt,
                                 const Mat& img, const Point* pattern,         /// pattern = pattern + 16 计算描述子需要的模式
                                 ...
                                 uchar* desc)
{
    ...
    // step 在这里指一行元素的大小，单位字节
    const int step = (int)img.step;


    // 二维旋转矩阵                                            /// use of define
#define GET_VALUE(idx) \
        center[cvRound(pattern[idx].x*b + pattern[idx].y*a)*step + \
               cvRound(pattern[idx].x*a - pattern[idx].y*b)]

    // 256对点来获取256*2bit的描述子
...
static int bit_pattern_31_[256*4] =                                          /// correlation
{
    ...
    iniThFAST(_iniThFAST), minThFAST(_minThFAST)
{
    ...
    // 总共期望提取 nfeatures 个特征点，根据尺度因子等比数列，计算出金字塔最底层期望提取的特征点个数
    ...
    const Point* pattern0 = (const Point*)biglet_pattern_31_;
    // 复制训练的模板 copy把序列pattern0，复制到pattern容器中
    std::copy(pattern0, pattern0 + npoints, std::back_inserter(pattern));

    // 区域圆的半径，umax 存储圆边上每一个像素点横坐标v对应的纵坐标
    umax.resize(HALF_PATCH_SIZE + 1);
    // 向下取整 vmax 以HALF_PATCH_SIZE为直角边的等腰三角形边长
...
{
    const int halfX = ceil(static_cast<float>(UR.x-UL.x)/2);                /// static_cast<float>, const
    const int halfY = ceil(static_cast<float>(BR.y-UL.y)/2);                /// UR BR UL BL
                                       ...
                                       const int &maxX, const int &minY, const int &maxY, const int &N, const int &level)
{
    ...
    for(int i=0; i<nIni; i++)
    {
        ...
        ni.BR = cv::Point2i(ni.UR.x,maxY-minY);                            /// maxX minX 怎样取值
    ...
    vector<pair<int,ExtractorNode*> > vSizeAndPointerToNode;                  /// vector<pair>
    ...
    while(!bFinish)
    {
        ...
        */
            while(!bFinish)
            {
                ...
                // 对需要划分的部分进行排序, 即对特征点数较少的区域进行优先划分
    ...
    for(list<ExtractorNode>::iterator lit=lNodes.begin(); lit!=lNodes.end(); lit++)
    {
        ...
        float maxResponse = pKP->response;                /// how to find the response
...
void ORBextractor::ComputeKeyPointsOctTree(vector<vector<KeyPoint> >& allKeypoints)
{
    ...
    for (int level = 0; level < nlevels; ++level)
    {
        ...
        // 根据特征点数目判断是否需要减小FAST阈值，需要对每一小块区域进行判断。
        ...
        */
        ...
        for(int i=0; i<nkps ; i++)
        {
            ...
            keypoints[i].size = scaledPatchSize;             /// size 尺度信息的作用
        }
    }
...
{ 
    ...
    Mat image = _image.getMat();                     /// InputArray operator
    ...
    {
        _descriptors.create(nkeypoints, 32, CV_8U);         /// Outputarray creat
    ...
    for (int level = 0; level < nlevels; ++level)
    {
        ...
        if( level != 0 )
        {
            ...
            copyMakeBorder(mvImagePyramid[level], temp, EDGE_THRESHOLD, EDGE_THRESHOLD, EDGE_THRESHOLD, EDGE_THRESHOLD,
                           BORDER_REFLECT_101+BORDER_ISOLATED);            /// 边界对于金字塔的作用是什么
        }
        else
        {
            copyMakeBorder(image, temp, EDGE_THRESHOLD, EDGE_THRESHOLD, EDGE_THRESHOLD, EDGE_THRESHOLD,
                           BORDER_REFLECT_101);           // 0 level 金字塔图片随temp变化而变化
        }
    }
